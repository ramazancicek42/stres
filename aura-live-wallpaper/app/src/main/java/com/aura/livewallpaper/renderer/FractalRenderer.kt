...
class FractalRenderer(
    ...
    // Renk paletleri (colors.xml'den alınacak)
    ...
    override fun onDrawFrame(gl: GL10?) {
        ...
        // Touch intensity zamanla azalır
        val timeSinceTouch = (System.currentTimeMillis() - lastTouchTime) / 1000f
        val decayedTouchIntensity = touchIntensity * (1.0f - timeSinceTouch.coerceIn(0f, 1f))
    ...
    fun setLightLevel(lux: Float) {
        // LUX değerini normalize et (0-10000 -> 0-1)
    ...
    fun handleTouchEvent(event: MotionEvent): Boolean {
        when (event.action) {
            MotionEvent.ACTION_DOWN, MotionEvent.ACTION_MOVE -> {
                touchX = event.x / (event.rawXRange()?.toFloat() ?: 1f)
                touchY = event.y / (event.rawYRange()?.toFloat() ?: 1f)
                touchIntensity = 1f
...
// Helper extension for MotionEvent raw coordinate range estimation
private fun MotionEvent.rawXRange(): IntRange? = null
private fun MotionEvent.rawYRange(): IntRange? = null
